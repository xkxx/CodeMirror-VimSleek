...
(function() {
  ...
  var defaultKeymap = [
    ...
    { keys: ['<C-->'], type: 'keyToKey', toKeys: ['<C-o>']},
    { keys: ['<C-=>'], type: 'keyToKey', toKeys: ['<C-i>']},
    { keys: ['U'], type: 'keyToKey', toKeys: ['<C-r>'] },
    { keys: ['<Home>'], type: 'keyToKey', toKeys: ['0'] },
    { keys: ['<End>'], type: 'keyToKey', toKeys: ['$'] },
    { keys: ['<C-k>'], type: 'keyToKey', toKeys: ['<C-b>'] },
    { keys: ['<C-j>'], type: 'keyToKey', toKeys: ['<C-f>'] },
    ...
    { keys: ['s', 'character', 'character'], type: 'motion',
        motion: 'seek',
        motionArgs: { forward: true , inclusive: true, toJumplist: true }},
    { keys: ['S', 'character', 'character'], type: 'motion',
        motion: 'seek',
        motionArgs: { forward: false, toJumplist: true }},
    { keys: ['f', 'character', 'character'], type: 'motion',
        motion: 'seekInPage',
        motionArgs: { forward: true , inclusive: true, pagewise: true, toJumplist: true }},
    { keys: ['F', 'character', 'character'], type: 'motion',
        motion: 'seekInPage',
        motionArgs: { forward: false, pagewise: true, toJumplist: true }},
    { keys: ['t', 'character', 'character'], type: 'motion',
        motion: 'seekTill',
        motionArgs: { forward: true, inclusive: true, toJumplist: true }},
    { keys: ['T', 'character', 'character'], type: 'motion',
        motion: 'seekTill',
  ...
  var Vim = function() {
    ...
    var motions = {
      ...
      seekTill: function(cm, motionArgs) {
        var repeat = motionArgs.repeat;
        var curEnd = seek(cm, repeat, motionArgs.forward,
      ...
      seek: function(cm, motionArgs) {
        var repeat = motionArgs.repeat;
        recordLastCharacterSearch(0, motionArgs);
        return seek(cm, repeat, motionArgs.forward,
            motionArgs.selectedCharacter) || cm.getCursor();
      },
      seekInPage: function(cm, motionArgs) {
        var repeat = motionArgs.repeat;
        recordLastCharacterSearch(0, motionArgs);
        return seekInPage(cm, repeat, motionArgs.forward,
            motionArgs.selectedCharacter) || cm.getCursor();
      },
      ...
      repeatLastCharacterSearch: function(cm, motionArgs) {
        ...
        var curEnd;
        if (lastSearch.pagewise === true) {
            curEnd = seekInPage(cm, repeat, forward, lastSearch.selectedCharacter);
        }
        else {
            curEnd = seek(cm, repeat, forward, lastSearch.selectedCharacter);
        }
        if (!curEnd) {
          cm.moveH(increment, 'char');
          return cm.getCursor();
    ...
    function recordLastCharacterSearch(increment, args) {
        ...
        vimGlobalState.lastChararacterSearch.pagewise = args.pagewise;
    }
    ...
    function seek(cm, repeat, forward, character) {
      var cur = cm.getCursor();
      var start = cur.ch;
      var idx;
      for (var i = 0; i < repeat; i ++) {
    ...
    function seekInPage(cm, repeat, forward, character) {
      var cur = cm.getCursor();
      var start = cur.ch;
      var currentLine = cur.line;
      var pageStart = cm.display.showingFrom;
      var pageEnd = cm.display.showingTo;
      var idx;
      

      for (var i = 0; i < repeat; i ++) {
        while(true) {
          var line = cm.getLine(currentLine);
          idx = charIdxInLine(start, line, character, forward, true);
          if (idx == -1) {
            if (forward === true && currentLine+1 < pageEnd) {
              currentLine ++;
              start = 0;
            }
            else if (forward === false && currentLine > pageStart) {
              currentLine --;
              start = Infinity;
            }
            else {
      ...
      return { line: currentLine, ch: idx };
    }
    ...
    var exCommands = {
      ...
      substitute: function(cm, params) {
        ...
        if (flagsPart) {
          ...
          if (flagsPart.indexOf('g') != -1) { // global
            lineStart = 0;
            lineEnd = cm.lastLine();
            flagsPart.replace('g', '');
          }
